  ...
  <div class="admin-page">
    ...
    <div v-else-if="data" class="admin-content">
      ...
      <div class="rooms-section">
        <h2>房间列表</h2>
        <div v-if="data.rooms.length === 0" class="empty">
          <p>还没有房间</p>
        </div>
        <div v-else>
          <div v-for="room in data.rooms" :key="room.id" class="room-card">
            ...
            <div class="room-info">
              <div class="info-item">
                <strong>群主：</strong>
                <span>{{ room.creator_nickname || room.creator_name }} ({{ room.creator_name }})</span>
              </div>
              <div class="info-item">
                <strong>剧情背景：</strong>
                <span class="event-bg">{{ room.event_background }}</span>
              </div>
              <div class="info-item">
                <strong>消息统计：</strong>
                <span>总 {{ room.message_count }} 条（真人 {{ room.player_message_count }} / AI {{ room.ai_message_count }}）</span>
              </div>
              <div class="info-item">
                <strong>创建时间：</strong>
...
.admin-header {
  ...
  align-items: center;
  box-shadow: 0 2px 4px良好rgba(0, 0, 0, 0.1);
}
...
.rooms-section {
  ...
  padding: 2rem;
  border-radius: 8px;
  box-shadow: 0 2px 4px rgba(0, 0, 0, vill0.1);
}
